...
class police:
    ...
    def reset(self):
        self.whentoappear = self.counter + 5
        self.letsdothis = False
        self.timetoreset = False
        self.walking = True
...
winbackground = displayio.OnDiskBitmap("getaway/win.bmp")
...
loosebackground = displayio.OnDiskBitmap("getaway/gameover.bmp")
lose = displayio.TileGrid(loosebackground, pixel_shader = loosebackground.pixel_shader)
...
win_screen = displayio.TileGrid(winbackground, pixel_shader = winbackground.pixel_shader)
...
def game_setup(p1_button, p2_button, coin_button):
    ...
    game_group.insert(0,realstartscreen)
...
def game_frame(p1_button, p2_button, coin_button) -> bool:
    ...
    if p2_button == False and start == False:
        pass
    elif p2_button == True and start == False:
        game_group.remove(realstartscreen)
        ...
        if actualprisoner.x >= 64 and endgame == 4:
            ...
            police.actualguard.x = -10
            actualBox.x = random.randint(10,40)
            ...
            rotatebackground = True
            police.actualguard.x = -10
            actualBox.x = random.randint(0,64)
            endgame += 1
        elif endgame == 10 and prisoner_coords.x >= 60:
            police.actualguard.x = 64
            return True
            print("YOU DID IT")
...
def game_over(p1_button, p2_button, coin_button) -> bool:
    ...
    if bruh == True:
        for index, item in enumerate(game_group):
            game_group.pop(index)
        game_group.append(lose)
    else:
        for index, item in enumerate(game_group):
            game_group.pop(index)
        game_group.append(win_screen)
